...
Public Class frmMantNacionalitats
    Dim blEnable As Boolean
    Dim blnNouRecord As Boolean

    Const TAULA_ID As TAULES_DBS = TAULES_DBS.TAULA_NACIONALITATS
    Const FORM_DESCRIP As String = "Manteniment Nacionalitats"
    Const CONNEX_DESCRIP As String = "Nacionalitats"
    ...
    Private Sub SaveRecord()
        ...
        For i As Short = 0 To 4
            ...
            If IsLoadForm("frmDiscos") Then
                combo = frmDiscos.ComboNacions
                OmpleCombo(combo, TAULA_ID, TotsCap.CAP)
                combo.SelectedValue = ID
                OmpleCombo(frmDiscos.gridComboNacions, TAULA_ID, TotsCap.CAP)
            End If
        Next i
...
#End Region
    ...
    Private Sub TabControlMesInfo_MouseUp(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles TabControlMesInfo.MouseUp
        If TabControlMesInfo.SelectedIndex = 1 Then
            Dim db As New MSC.db.clsDBS
            Dim StrSql As String = "SELECT discID, discNom FROM `discos` where discNacionalID =" & Me.lbID.Text
            Dim dt As DataTable = db.getTable(StrSql)
            Me.DataGridViewAnuncis.DataSource = dt : DataGridViewAnuncis.Columns(1).HeaderText = "Nom ( " & dt.Rows.Count & " registres) " : DataGridViewAnuncis.Columns(1).HeaderText = "Nom ( " & dt.Rows.Count & " registres) "
