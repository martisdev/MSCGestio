...
Public Class frmMantProductes
	
	Dim blEnable As Boolean
	Dim blnNouRecord As Boolean
	Dim IsSaving As Boolean = False
	
	Const TAULA_ID As TAULES_DBS = TAULES_DBS.TAULA_PRODUCTES
    ...
    Dim FORM_DESCRIP As String = "Manteniment Productes o Serveis"
	Dim CONNEX_DESCRIP As String = "Productes o Serveis"
	
	
	Public Sub setLanguageForm()
		lang.StrForm = Me.Name
		
		FORM_DESCRIP = lang.getText("Text") '"Manteniment Productes o Serveis"
		CONNEX_DESCRIP = lang.getText("CONNEX_DESCRIP") '"Productes o Serveis"
		Me.TabPage1.Text = lang.getText("TabPage1.Text") '""LListat del producte""
		Me.TabPage2.Text = lang.getText("TabPage2.Text") '"Anuncis del producte"		
	...
	Private Sub DeleteRecord()
		...
		For i As Short = 0 To 1
			...
			If IsLoadForm("frmPubli") Then
				MyCombo = frmPubli.ComboProducte
				Dim Cmb As New combo
				Cmb.OmpleCombo(MyCombo, TAULA_ID, TotsCap.CAP)
				Cmb.OmpleCombo(frmPubli.ComboGridProducte, TAULA_ID, TotsCap.CAP)
			End If
		Next i
	...
	Private Sub CloseForm()
		...
		MyAPP.CloseSesionClient(MyAPP.IDSesion_Client, DESCRIP_FORM_PRODUCTES)
	...
	Private Sub TabControlMesInfo_MouseUp(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles TabControlMesInfo.MouseUp
		If TabControlMesInfo.SelectedIndex = 1 Then
            Dim db As New MSC.dbs(Cloud)
            Dim StrSql As String = "SELECT falc_id, falc_nom FROM `falques` where falc_producte =" & Me.lbID.Text
