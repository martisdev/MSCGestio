...
Public Class frmMantEstil
	...
	Private Sub frmMant_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If My.Settings.TXT_CAPITALIZE Then 
			...
			Me.txtDescrip.CharacterCasing = CharacterCasing.Upper
			'Me.comboBoxEstilPare.CharacterCasing = CharacterCasing.Upper
		End If
    ...
    Dim FORM_DESCRIP As String = "Manteniment Estils"
	Dim CONNEX_DESCRIP As String = "Estils"
	
	Public Sub setLanguageForm()
		lang.StrForm = Me.Name
		
		FORM_DESCRIP = lang.getText("Text") '"Manteniment Estils"
		CONNEX_DESCRIP = lang.getText("LABEL_ESTILS",True) '"Estils"
		Me.TabPage1.Text = lang.getText("TabPage1.Text") '"LListat estils"
		Me.TabPage2.Text = lang.getText("TabPage2.Text") '"Discos d'aquest estil"
		Me.TabPage3.Text = lang.getText("TabPage3.Text") '"Cançons d'aquest estil"
		...
		Me.label3.Text = lang.getText("LABEL_DESCRIPCIO",True) & ":" '"Descripció"
		Me.label4.Text = lang.getText("label4.Text") & ":" '"Estil Pare"
        ...
        Cmb.OmpleCombo(comboBoxEstilPare, TAULES_DBS.TAULA_ESTILS, TotsCap.CAP) : comboBoxEstilPare.SelectedValue = 0
        Cmb = Nothing
	...
	Private Sub gridDades_CurrentCellChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles gridDades.CurrentCellChanged
		...
		Try
			...
			Me.txtDescrip.Text  = StripSlashes(NoNul(gridDades.CurrentRow.Cells("ColumnDescrip").Value))
			Me.chkVisible.Checked =   CBool(gridDades.CurrentRow.Cells("ColumnVisible").Value)
			Me.comboBoxEstilPare.SelectedValue = CInt(gridDades.CurrentRow.Cells("ColumnPare").Value)
	...
	Friend Sub establirRecordset(Optional ByVal Condi As String = "")
		...
		Dim StrSql As String = "SELECT  `estil_id`,`estil_nom`,`estil_descrip`,`estil_pare`,`estil_visible` FROM `estils`"
    ...
    Private Sub SaveRecord()
		...
		Dim Visibles As Boolean = Me.chkVisible.Checked 
		Dim StlPare As Integer = Me.comboBoxEstilPare.SelectedValue

        Dim db As New MSC.dbs(Cloud)
        If blnNouRecord = True Then
			Dim StrSql As String = "INSERT INTO `estils` ( `estil_nom`,`estil_descrip`,`estil_pare`,`estil_visible` )" & _
				"VALUES ('" & Nom & "','"& Descrip &"'," & StlPare & ", "& Visibles &");"
			ID = db.New_ID(StrSql)
		...
		Else
			Dim StrSql As String = "UPDATE `estils` SET " & _
				"`estil_nom`='" & Nom & "'" & _
				",`estil_descrip`= '" & Descrip & "'" & _
				",`estil_pare`= " & StlPare & _
				",`estil_visible`= " & Visibles & _
				" WHERE " & COLUMN_ID & " = " & ID
        ...
        Dim Cmb As New combo
        Dim IdCmb As Integer = comboBoxEstilPare.SelectedValue
		Cmb.OmpleCombo(comboBoxEstilPare, TAULES_DBS.TAULA_ESTILS, TotsCap.CAP) : comboBoxEstilPare.SelectedValue = IdCmb 
	...
	Private Sub CloseForm()
        ...
        MyAPP.CloseSesionClient(MyAPP.IDSesion_Client, DESCRIP_FORM_ESTIL)
    End Sub
	...
	Private Sub LockedRecord(Optional ByVal saved As Boolean = False)
		...
		Me.comboBoxEstilPare.Enabled = blEnable
	...
	Private Sub TabControlMesInfo_MouseUp(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles TabControlMesInfo.MouseUp
        ...
        If TabControlMesInfo.SelectedIndex = 1 Then
			Dim StrSql As String = "SELECT disc_id, disc_nom FROM `discos` where disc_estil =" & Me.lbID.Text
			Me.DataGridViewAnuncis.DataSource = db.getTable(StrSql) : DataGridViewAnuncis.Columns(1).HeaderText =  Me.ColumnNom.HeaderText & " ( " & DataGridViewAnuncis.Rows.Count & " " & LABEL_REGISTRES & ")"
		ElseIf TabControlMesInfo.SelectedIndex = 2 Then
			'Dim StrSql As String = "SELECT tema_id, tema_titol FROM `temes` where tema_estil =" & Me.lbID.Text
			Dim StrSql As String = "SELECT temes.tema_id, temes.tema_titol " & _
										"FROM temes " & _
										"  INNER JOIN style_relationships " & _
										"    ON temes.tema_id = style_relationships.style_relationships_fitxer " & _
										"WHERE style_relationships.style_relationships_tipfitxer = " & Tipus_Fitxers.FITXER_MUSICA & " " & _
										"AND style_relationships.style_relationships_fitxer = temes.tema_id " & _
										"AND style_relationships.style_relationships_style = " & lbID.Text & ";"
    ...
    Sub TxtDescripTextChanged(sender As Object, e As EventArgs) Handles txtDescrip.TextChanged
        Try
            Dim RowID As Integer = FindAfectatRow()
            Me.gridDades.Rows(RowID).Cells("ColumnDescrip").Value = txtDescrip.Text
            Changed()
        Catch ex As Exception
        End Try
    End Sub

    Sub ChkVisibleCheckedChanged(sender As Object, e As EventArgs) Handles chkVisible.CheckedChanged
        Try
            Dim RowID As Integer = FindAfectatRow()
    ...
    Sub ComboBoxEstilPareSelectedIndexChanged(sender As Object, e As EventArgs) Handles comboBoxEstilPare.SelectedIndexChanged
        Try
            Dim RowID As Integer = FindAfectatRow()
            Me.gridDades.Rows(RowID).Cells("ColumnPare").Value = comboBoxEstilPare.SelectedValue
            Changed()
        Catch ex As Exception
        End Try
    End Sub

    Sub ComboBoxEstilPareKeyUp(sender As Object, e As KeyEventArgs) Handles comboBoxEstilPare.KeyUp
        refreshComboBoxKeyUp(comboBoxEstilPare, TAULES_DBS.TAULA_ESTILS, TotsCap.CAP)
    End Sub

    Sub ComboBoxEstilPareKeyPress(sender As Object, e As KeyPressEventArgs) Handles comboBoxEstilPare.KeyPress
