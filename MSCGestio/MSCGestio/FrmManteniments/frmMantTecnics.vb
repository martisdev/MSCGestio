...
Public Class frmMantTecnics
    Dim blEnable As Boolean
    Dim blnNouRecord As Boolean
    Dim IsSaving As Boolean = False

    Const TAULA_ID As TAULES_DBS = TAULES_DBS.TAULA_TECNICS
...
Private Sub frmMant_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
	...
	Dim FORM_DESCRIP As String = "Manteniment de Tècnics"
	Dim CONNEX_DESCRIP As String = "Tècnics"
	
	Public Sub setLanguageForm()
		lang.StrForm = Me.Name
		
		FORM_DESCRIP = lang.getText("Text") '"Manteniment de Tècnics"
		CONNEX_DESCRIP = lang.getText("LABEL_TECNICS",True) '"Tècnics"
		Me.TabPage1.Text = lang.getText("TabPage1.Text") '""LListat de tècnics""
		Me.TabPage1.Text = lang.getText("TabPage2.Text") '"Anuncis del tècnic"
...
#Region " Funcions internes"
    ...
    Private Sub SaveRecord()
        ...
        If IsLoadForm("frmPubli") Then
            MyCombo = frmPubli.ComboTecnic
            Dim Cmb As New combo
            Cmb.OmpleCombo(MyCombo, TAULA_ID, TotsCap.CAP)
            MyCombo.SelectedValue = ID
            Cmb.OmpleCombo(frmPubli.ComboGridTecnic, TAULA_ID, TotsCap.CAP)
        End If
...
Private Sub DeleteRecord()
    ...
    Private Sub CloseForm()
        ...
        MyAPP.CloseSesionClient(MyAPP.IDSesion_Client, DESCRIP_FORM_TECNIC)
        Me.Dispose()
    End Sub
...
#End Region
    ...
    Private Sub TabControlMesInfo_MouseUp(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles TabControlMesInfo.MouseUp
        If TabControlMesInfo.SelectedIndex = 1 Then
            Dim db As New MSC.dbs
            Dim StrSql As String = "SELECT falc_id, falc_nom FROM `falques` where falc_tecnic =" & Me.lbID.Text
