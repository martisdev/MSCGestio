...
Public Class frmMantLocutors
	Dim blEnable As Boolean
	Dim blnNouRecord As Boolean
	Dim IsSaving As Boolean = False
	
	Const TAULA_ID As TAULES_DBS = TAULES_DBS.TAULA_LOCUTORS
    ...
    Dim FORM_DESCRIP As String = "Manteniment Locutors"
	Dim CONNEX_DESCRIP As String = "Locutors"
	
	
	Public Sub setLanguageForm()
		lang.StrForm = Me.Name
		
		FORM_DESCRIP = lang.getText("Text") '"Manteniment Locutors"
		CONNEX_DESCRIP = lang.getText("LABEL_LOCUTORS",True) '"Locutors"
		Me.TabPage1.Text = lang.getText("TabPage1.Text") '""LListat de Locutors""
		Me.TabPage2.Text = lang.getText("TabPage2.Text") '"Anuncis del locutor"		
    ...
    Private Sub DeleteRecord()
		...
		For i As Short = 0 To 1
			...
			If IsLoadForm("frmPubli") Then
				MyCombo = frmPubli.ComboLocutor
				Dim Cmb As New combo
				Cmb.OmpleCombo(MyCombo, TAULA_ID, TotsCap.CAP)
				Cmb.OmpleCombo(frmPubli.ComboGridLocutor, TAULA_ID, TotsCap.CAP)
			End If
		Next i
	End Sub
	...
	Private Sub CloseForm()
        ...
        MyAPP.CloseSesionClient(MyAPP.IDSesion_Client, DESCRIP_FORM_LOCUTOR)
    End Sub
	...
	Private Sub TabControlMesInfo_MouseUp(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles TabControlMesInfo.MouseUp
		If TabControlMesInfo.SelectedIndex = 1 Then
            Dim db As New MSC.dbs(Cloud)
            Dim StrSql As String = "SELECT falc_id, falc_nom FROM `falques` where falc_locutor =" & Me.lbID.Text
	...
	Private Sub mnu_Help_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnu_Help.Click
		...
		Try
			Process.Start(MSC.Help.HELP_GEST_PUBLICITAT_LOCUTOR.ToString)
		Catch ex As Exception
		End Try
	End Sub
End Class