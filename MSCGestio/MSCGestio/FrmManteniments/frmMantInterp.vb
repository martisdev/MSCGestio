...
Public Class frmMantInterp
	
	Dim blEnable As Boolean
	Dim blnNouRecord As Boolean
	Dim IsSaving As Boolean = False
	
	Const TAULA_ID As TAULES_DBS = TAULES_DBS.TAULA_INTERPRETS
...
#Region " Pantalla "
    ...
    Dim FORM_DESCRIP As String = "Manteniment Intèrprets"
    Dim CONNEX_DESCRIP As String = "Intèrprets"
	
	Public Sub setLanguageForm()
		lang.StrForm = Me.Name
		
		FORM_DESCRIP = lang.getText("Text") '"Manteniment Intèrprets"
		CONNEX_DESCRIP = lang.getText("LABEL_INTERPRETS",True) '"Intèrprets"
		Me.TabPage1.Text = lang.getText("TabPage1.Text") '""LListat Intèrprets""
		Me.TabPage2.Text = lang.getText("TabPage2.Text") '"Discos d'aquest intèrpret"
		Me.TabPage3.Text = lang.getText("TabPage3.Text") '"Cançons d'aquest intèrpret"
		Me.TabPage4.Text = lang.getText("LABEL_ULTIMES_CANCONS_RADIADES",True) '"Últimes cançons radiades"
...
#Region " Funcions internes"
	...
	Private Sub DeleteRecord()
		...
		For i As Short = 0 To 2
			...
			Select Case i
				Case 0 : If IsLoadForm("frmDiscos") Then MyCombo = frmDiscos.ComboInterp
				Case 1 : If IsLoadForm("frmTemes") Then MyCombo = frmTemes.ComboInterp
				Case 2 : If IsLoadForm("frmImport") Then MyCombo = frmImport.cbInterp
			End Select
	...
	Private Sub CloseForm()
        ...
        MyAPP.CloseSesionClient(MyAPP.IDSesion_Client, DESCRIP_FORM_INTERPRET)
    End Sub
	...
	Private Sub TabControlMesInfo_MouseUp(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles TabControlMesInfo.MouseUp
        ...
        If TabControlMesInfo.SelectedIndex = 1 Then
			Dim StrSql As String = "SELECT disc_id, disc_nom FROM `discos` where disc_interpret =" & Me.lbID.Text
		...
		ElseIf TabControlMesInfo.SelectedIndex = 2 Then
			Dim StrSql As String = "SELECT tema_id, tema_titol FROM `temes` where tema_interp =" & Me.lbID.Text
		...
		ElseIf TabControlMesInfo.SelectedIndex = 3 Then
			Dim StrSql As String = "SELECT temes.tema_id, temes.tema_titol, controlradiacio.ctl_dataradiacio" _
				& " FROM (controlradiacio INNER JOIN temes ON controlradiacio.ctl_fitxer = temes.tema_id) INNER JOIN interprets ON temes.tema_interp = interprets.interp_id" _
				& " WHERE(((controlradiacio.ctl_tipfitxer) = 2) And interp_id = " & lbID.Text & ")" _
				& " ORDER BY controlradiacio.ctl_dataradiacio;"
			
			Me.DataGridViewRadiacio.DataSource = db.getTable(StrSql) : DataGridViewRadiacio.Columns(1).HeaderText = Me.ColumnNom.HeaderText & " ( " & DataGridViewRadiacio.Rows.Count &  " " & LABEL_REGISTRES & ")"
		End If
		db = Nothing
	End Sub
