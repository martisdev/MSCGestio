...
Public Class frmMantTarifa
    ...
    Dim FORM_DESCRIP As String = "Manteniment Tarifes"
	Dim CONNEX_DESCRIP As String = "Tarifes"
	
	Public Sub setLanguageForm()
		lang.StrForm = Me.Name
		
		FORM_DESCRIP = lang.getText("Text") '"Manteniment Tarifes"
		CONNEX_DESCRIP = lang.getText("LABEL_TARIFES",True) '"Tarifes"
		Me.TabPage1.Text = lang.getText("TabPage1.Text") '""LListat Tarifes""
		Me.TabPage1.Text = lang.getText("TabPage2.Text") '"Anuncis amb aquesta tarifa"
		Me.Label4.Text = lang.getText("LABEL_PREU",True) 'Preu
	...
	Private Sub gridDades_CurrentCellChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles gridDades.CurrentCellChanged
		...
		Try
			...
			Me.txtDescrip.Text = StripSlashes(gridDades.CurrentRow.Cells("ColumnDescrip").Value)
	...
	Private Sub txtPreu_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtPreu.TextChanged
		Try
			Dim RowID As Integer = FindAfectatRow()
			Me.gridDades.Rows(RowID).Cells("ColumnPreu").Value = AddSlashes(txtPreu.Text)
			changed()
		Catch ex As Exception
		End Try
		Try
    ...
    Private Sub SaveRecord()
        ...
        If blnNouRecord = True Then
            StrSql = "INSERT INTO tarifes (`tarif_nom`, `tarif_descrip`, `tarif_preu`)"
        ...
        Else
            ...
            StrSql = StrSql & " SET "
            StrSql = StrSql & " tarif_descrip = '" & Descrip & "'"
            StrSql = StrSql & ", tarif_nom = '" & Nom & "'"
            StrSql = StrSql & ", `tarif_preu` = '" & Preu & "'"
            StrSql = StrSql & " WHERE `tarif_id` = " & ID & " ;"
    ...
    Private Sub DeleteRecord()
		...
		For i As Short = 0 To 1
			...
			If IsLoadForm("frmPubli") Then
				MyCombo = frmPubli.ComboTarifes
				Dim Cmb As New combo
				Cmb.OmpleCombo(MyCombo, TAULA_ID, TotsCap.CAP)
				MyCombo.SelectedValue = 1
				Cmb.OmpleCombo(frmPubli.ComboGridTarifa, TAULA_ID, TotsCap.CAP)
			End If
		Next i
	...
	Private Sub CloseForm()
        ...
        MyAPP.CloseSesionClient(MyAPP.IDSesion_Client, DESCRIP_FORM_TARIFES)
    End Sub
	...
	Private Sub TabControlMesInfo_MouseUp(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles TabControlMesInfo.MouseUp
		If TabControlMesInfo.SelectedIndex = 1 Then
            Dim db As New MSC.dbs(Cloud)
            Dim StrSql As String = "SELECT falc_id, falc_nom FROM `falques` where falc_tarifa =" & Me.lbID.Text
	...
	Private Sub mnu_Help_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnu_Help.Click
		...
		Try
			Process.Start(MSC.Help.HELP_GEST_PUBLICITAT_TARIFES.ToString)
		Catch ex As Exception
		End Try
	End Sub
End Class