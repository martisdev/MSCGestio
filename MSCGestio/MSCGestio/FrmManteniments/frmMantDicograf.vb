...
Public Class frmMantDicograf
	Dim blEnable As Boolean
	Dim blnNouRecord As Boolean
	Dim IsSaving As Boolean = False
	
	Const TAULA_ID As TAULES_DBS = TAULES_DBS.TAULA_DISCOGRAFIQUES
    ...
    Dim FORM_DESCRIP As String = "Manteniment Discogràfiques"
	Dim CONNEX_DESCRIP As String = "Companyies Discogràfiques"
	
	Public Sub setLanguageForm()
		lang.StrForm = Me.Name
		
		FORM_DESCRIP = lang.getText("Text")' "Manteniment Discogràfiques"
		CONNEX_DESCRIP = lang.getText("LABEL_COMPANY_DISCOS",True)'"Companyies Discogràfiques"
		Me.TabPage1.Text = lang.getText("TabPage1.Text")'"LListat de Discogràfiques"
		Me.TabPage2.Text = lang.getText("TabPage2.Text")'"Discos d'aquesta discogràfica"
    ...
    Private Sub DeleteRecord()
		...
		For i As Short = 0 To 1
			...
			If IsLoadForm("frmDiscos") Then
				MyCombo = frmDiscos.ComboCia
				Dim Cmb As New combo
				Cmb.OmpleCombo(MyCombo, TAULA_ID, TotsCap.CAP)
				Cmb.OmpleCombo(frmDiscos.gridComboCia, TAULA_ID, TotsCap.CAP)
			End If
		Next i
	...
	Private Sub CloseForm()
        ...
        MyAPP.CloseSesionClient(MyAPP.IDSesion_Client, DESCRIP_FORM_DISCOGRAFIQUES)
    End Sub
	...
	Private Sub TabControlMesInfo_MouseUp(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles TabControlMesInfo.MouseUp
		If TabControlMesInfo.SelectedIndex = 1 Then
            Dim db As New MSC.dbs(Cloud)
            Dim StrSql As String = "SELECT disc_id, disc_nom FROM `discos` where disc_cia =" & Me.lbID.Text
