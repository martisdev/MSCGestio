...
Public Class frmMantInterp
    ...
    Const FORM_DESCRIP As String = "Manteniment Intèrprets"
    Const CONNEX_DESCRIP As String = "Intèrprets"
...
#Region " Funcions internes"
    ...
    Private Sub SaveRecord()
        ...
        For i As Short = 0 To 2
            ...
            Select Case i
                Case 0 : If IsLoadForm("frmDiscos") Then combo = frmDiscos.ComboInterp
                Case 1 : If IsLoadForm("frmTemes") Then combo = frmTemes.ComboInterp
                Case 2 : If IsLoadForm("frmImport") Then combo = frmImport.cbInterp                
            End Select
...
#End Region
    ...
    Private Sub TabControlMesInfo_MouseUp(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles TabControlMesInfo.MouseUp
        If TabControlMesInfo.SelectedIndex = 1 Then
            Dim db As New MSC.db.clsDBS
            Dim StrSql As String = "SELECT discID, discNom FROM `discos` where discInterpID =" & Me.lbID.Text
        ...
        ElseIf TabControlMesInfo.SelectedIndex = 2 Then
            Dim db As New MSC.db.clsDBS
            Dim StrSql As String = "SELECT temID, temTitol FROM `temes` where temInterpID =" & Me.lbID.Text
            Dim dt As DataTable = db.getTable(StrSql)            
            Me.DataGridViewTemes.DataSource = dt : DataGridViewTemes.Columns(1).HeaderText = "Nom ( " & dt.Rows.Count & " registres) "
        ElseIf TabControlMesInfo.SelectedIndex = 3 Then
            Dim db As New MSC.db.clsDBS
            Dim StrSql As String = "SELECT temes.temID, temes.temTitol, controlradiacio.ctlDataRadiacio" _
            & " FROM (controlradiacio INNER JOIN temes ON controlradiacio.ctlFitxerID = temes.temID) INNER JOIN interprets ON temes.temInterpID = interprets.interpID" _
            & " WHERE(((controlradiacio.ctlTipFitxer) = 2) And interpID = " & lbID.Text & ")" _
            & " ORDER BY controlradiacio.ctlDataRadiacio;"
            Dim dt As DataTable = db.getTable(StrSql)
            Me.DataGridViewRadiacio.DataSource = dt : DataGridViewRadiacio.Columns(1).HeaderText = "Nom ( " & dt.Rows.Count & " registres) "
        End If
    End Sub
